...
2. `wolves-intro` — the official YouTube IFrame Player embed of Bungie's "Destiny 2: Into the Light Cinematic" trailer (default unvoiced `youtubeVideoId: 'BV3BZKbpBns'`, optional voiced override `alternateYoutubeVideoId: 'BKm0TPqeOjY'`) with HTML/CSS character overlay text introducing the six Guardian characters. The shared intro widget exposes the exact `Ikora voice over` checkbox during this segment only, switching sources through object-form `loadVideoById({ videoId, startSeconds })` while preserving native time and pause state. Destiny dialogue captions are disabled; Guardian nameplates and the Comic Hero Shots card remain unconditionally visible. The user-authored post-Kaslin status (48.0 to 70.5 seconds) is hidden unless the bottom-player **CC** switch is enabled. Its top display reads **Meet your Fireteam** — **We fight for something bigger than ourselves.** The bottom media plaque remains **Destiny 2: Into the Light Cinematic**, except during the candle sequence (48.0 to 70.5 seconds), when it reads **#nova4ever**.
