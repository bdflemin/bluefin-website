...
When the user clicks the primary "Start soundtrack" / "Start Experience" button (the first `idle`/`error` -> playback transition in `WolvesSoundtrack.vue`, or the parent-driven `v-model:playing` transition used by `WolvesApp.vue`'s "JOIN THE EVOLUTION" button), a fullscreen intro video plays before the live YouTube playlist experience begins:

1. An official YouTube IFrame Player embed of Bungie's "Destiny 2: Into the Light Cinematic" trailer (`youtubeVideoId: 'BKm0TPqeOjY'`) with HTML/CSS text overlays introducing crew characters, mentioning other Guardians paired with dinosaurs (kept deliberately vague — see `[REDACTED]` mystery convention below). This is a legitimate embed via YouTube's own IFrame API — the exact same mechanism already used for every track in the soundtrack playlist — never a downloaded or re-encoded local copy of someone else's footage.
2. Then `startSoundtrack()` runs exactly as it always has, handing off into the normal live playlist experience (the separate `record-wolves.cjs` / `compile-slideshow.js` hero-video export pipeline is untouched by this system; it exists only for external YouTube upload, not for live embedding).

Architecture:

- `src/data/wolves-intro-sequence.ts`: pure, unit-tested state machine (`createIntroSequenceState`, `advanceIntroSequence`, `skipIntroSequence`, `activeOverlayText`) plus `buildIntroVideoSequence()`, the config listing each entry's `youtubeVideoId` and its character-overlay text cues (`{ text, start, end }`). This is the **content surface** for the intro sequence: the video id and overlay text/timing are editable with exact user-supplied copy.
...
- `src/components/wolves/WolvesIntroOverlay.vue`: fullscreen YouTube IFrame Player sequencer consuming that state machine. Constructs a `YT.Player` per video (`autoplay: 1, controls: 0, playsinline: 1, rel: 0, modestbranding: 1`), polls `getCurrentTime()` every 200ms to drive overlay text cues (the IFrame API has no `timeupdate` event), advances on `onStateChange` `ENDED`, auto-advances on `onError` (a load failure never blocks the live experience), and exposes a "Skip" button that jumps straight to the live experience. This component's markup/logic is locked, same as other Wolves components — only the data it's given (`wolves-intro-sequence.ts`) is editable.
...
**What agents may touch**: the `youtubeVideoId` and overlay text/timing in `buildIntroVideoSequence()` (`src/data/wolves-intro-sequence.ts`), with exact user-supplied copy only — same editorial rule as everywhere else on this page.

**What agents must not touch without a fresh, explicit user request**: `WolvesIntroOverlay.vue` markup/styles/logic, `useYoutubeIframeApi.ts`, the `handlePrimaryAction`/`props.playing` watcher/`handleIntroOverlayComplete` wiring in `WolvesSoundtrack.vue`, or the pure functions in `wolves-intro-sequence.ts` (only its `buildIntroVideoSequence()` data payload is content).
...
| `buildIntroVideoSequence()` in `src/data/wolves-intro-sequence.ts` | Intro-sequence `youtubeVideoId` and character-overlay text/timing cues; exact user-supplied copy only |
