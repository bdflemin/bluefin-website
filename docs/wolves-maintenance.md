...
2. `wolves-intro` — the official YouTube IFrame Player embed of Bungie's "Destiny 2: Into the Light Cinematic" trailer (default unvoiced `youtubeVideoId: 'BV3BZKbpBns'`, optional voiced override `alternateYoutubeVideoId: 'BKm0TPqeOjY'`) with HTML/CSS character overlay text introducing the six Guardian characters. The shared intro widget exposes the exact `Ikora voice over` checkbox during this segment only, switching sources through object-form `loadVideoById({ videoId, startSeconds })` while preserving native time and pause state. Destiny dialogue captions are disabled; Guardian nameplates, the Comic Hero Shots card, and the user-authored post-Kaslin status (48.0 to 70.5 seconds) remain. Its top display reads **Meet your Fireteam** — **We fight for something bigger than ourselves.** The bottom media plaque remains **Destiny 2: Into the Light Cinematic**.
