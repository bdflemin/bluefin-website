...
38. **Equinox Presentation Handoff:** Keep `presentationTrackIndex` unchanged while the Equinox overlay performs its 1.5-second opacity enter transition. Keep the Track 0 split grid rendered beneath that overlay, and source both layout/lore visibility and `WolvesComicReader` from this delayed presentation index. Apply the current playlist index only after that interval, then let the gallery appear beneath the still-active overlay and be revealed on its normal clear. Cancel and invalidate pending presentation callbacks on every track change and unmount so rapid changes cannot apply a stale index. Cover this timing with fake timers: Track 0 lore and comic remain mounted before 1.5 seconds and the later-track gallery replaces them after it.
