...
42. **Authored Quote Provenance:** Parse quote `attribution` and optional `context` frontmatter as strings. The quote view prefers these authored values over legacy source labels or titles; do not restore a duplicate permanent identity to the legacy artifact source map.
