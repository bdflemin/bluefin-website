...
- The locked final news view (398–425 seconds) must render its passed nonempty `warning` separately from `record.body` with a local `thesis-warning-fade` class: 20 seconds, linear, full opacity to `0.35`. Never substitute thesis `text` or `hudLabel`; cover the actual 405- and 425-second final-slot states.
...
42. **Authored Quote Provenance:** Parse quote `attribution` and optional `context` frontmatter as strings. An authored attribution is mandatory for every quote: emit a parser diagnostic and fail quote rendering rather than falling back to a legacy source label or title. The quote view renders both authored attribution and context. Do not restore a duplicate permanent identity to the legacy artifact source map; retain only source-record URL routing.

## Sources

- js-yaml: `/nodeca/js-yaml` (`load()` frontmatter parsing contract)
- Vue: `/vuejs/vue` (`v-if` conditional warning rendering and scoped component styles)
