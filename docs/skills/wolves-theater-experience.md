...
31. **Golden Era Climax:** For the existing BUR//S climax message in `lorem-pursuit-1`, derive its final sentence at runtime, hold it hidden for three seconds, then fade it in as a complete sentence before advancing to SARAH. This is delivery timing only; do not alter or duplicate source lore.
32. **Continuous Timeline Boundary Matching:** Resolve slots with `time < endTime`; at an exact boundary, advance to the next entry. Clamp the end of Track 0 and all later playback to the final entry.
