...
28. **Track 0 Narrative Timeline:** `wolvesNarrativeTimeline` is the sole narrative clock. It presents every `wolvesRelease.artifacts` entry once in release order across Track 0: the first entry begins at `0:00`, `lorem-pursuit-1` begins at `3:00` and holds for 40 seconds, and `blue-universal-acquires-wayland-yutani` begins at `6:38` and holds through `7:03`.
29. **Read-Only Graphic Novel:** The comic, lore, and soundtrack controls must not seek, skip, paginate, rotate, or otherwise change narrative position. The typewriter scales to each timeline slot and later soundtrack tracks retain the final narrative entry.
30. **Continuous Timeline Boundary Matching:** Resolve slots with `time < endTime`; at an exact boundary, advance to the next entry. Clamp the end of Track 0 and all later playback to the final entry.
