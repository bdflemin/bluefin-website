...
describe('wolves intro overlay sequence', () => {
  ...
  it('renames Robert Killen to Bob Killen without moving the authored guardian windows', () => {
    ...
    expect(destiny.overlays).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ text: 'Stormcaller Warlock — Kaslin Fields — Rage of the Paradox', start: 37, end: 48 }),
    ]))
  ...
  it('keeps the Destiny guardian wide plates authored with their wide-plate positioning flags', () => {
    ...
    expect(destiny.overlays).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ text: 'Broodweaver Warlock — Christoph Blecker — First Among Equals — The North Star', start: 82, end: 95, position: 'left', trustee: true, leader: true }),
      expect.objectContaining({ text: 'Behemoth Titan — Natali Vlatko — Shipwright of Kubernetes', start: 86.5, end: 96, position: 'right' }),
    ]))
