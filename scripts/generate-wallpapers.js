...
async function generate() {
  ...
  // Serialize to repository-lint-clean TypeScript: unquoted identifier keys and
  // single-quoted strings (JSON.stringify output fails style/quote-props and
  // style/quotes on a clean checkout).
  ...
  const serializeValue = (value, indent) => {
    ...
    if (value && typeof value === 'object') {
      ...
      const inner = entries
        .map(([key, v]) => {
          const safeKey = IDENTIFIER_RE.test(key) ? key : `'${key.replace(/'/g, '\\\'')}'`
...
export const wallpapers: Wallpaper[] = ${serializeValue(wallpapers, '')}
`
